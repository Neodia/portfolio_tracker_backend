...
use portfolio_tracker_backend::client::model::BlockchainAssetRate;
...
#[tokio::test]
async fn get_rates_per_network_http_fails() {
    let client = LiveCGClient::new("whatever_fake_server.com".into(), "fake_key".into());
    let response = client
        .get_rates_from_network(Network::Solana, vec!["whatever".into()])
        .await;
    assert!(matches!(response, Err(ClientError::HttpError(_))))
}

#[tokio::test]
async fn get_rates_per_network_unauthorized() {
    let mock_server = MockServer::start().await;
...
#[tokio::test]
async fn get_rates_per_network_rate_limited() {
    let mock_server = MockServer::start().await;
...
#[tokio::test]
async fn get_rates_per_network_not_found() {
    let mock_server = MockServer::start().await;
...
#[tokio::test]
async fn get_rates_per_network_unexpected() {
    let mock_server = MockServer::start().await;
